...
## Pipeline Steps
1. **Normalize** — total counts per cell scaled to 10,000
2. **Log1p** — log(x+1) transform
3. **HVG selection** — select top N highly variable genes (batch-aware)
4. **Scale** — zero mean, unit variance (max_value=10)
5. **PCA** — linear dimensionality reduction
6. **Harmony** — batch correction on PCA embedding using `batch_key`
7. **Neighbors** — k-NN graph on Harmony embedding
8. **UMAP** — non-linear dimensionality reduction
9. **Leiden clustering** — community detection
